...
if service syslog status | grep -E " active \(running";then
echo "syslog服务已开启"
else
...
if more /etc/rsyslog.conf | grep -E "@...\.|@..\.|@.\.|\*.\* @...\.|\*\.\* @..\.|\*\.\* @.\.";then
...
lsof -u root |grep -E "ESTABLISHED|SYN_SENT|LISTENING"
echo ----------------------------状态解释------------------------------
